...
static long FileLoadDrivers(char *dirSpec, long plugin, int dry_run);
static long FileLoadDriversMKext( char *dirSpec );
...
long LoadDrivers(char *dirSpec)
{
  ...
  if (gBootFileType == kNetworkDeviceType) {
    ...
    strcpy(str, gExtensionsSpec );

#if 0
    /* If the numbers of drivers is small then this is probably a boot-CD (almost
     * all drivers are only in the mkext). For a normal boot, we avoid the
     * mkext since it does not contain the NetworkFamily.
     */ 
    if( FileLoadDrivers(gExtensionsSpec, 0, 1 /* dry_run */) < 30 ) {
	    strcpy( gExtensionsSpec, str );
	    loaded = !FileLoadDriversMKext(gExtensionsSpec);
    }
#else
    loaded = !FileLoadDriversMKext(gExtensionsSpec);     
#endif
    if( !loaded ) {
	    strcpy( gExtensionsSpec, str );
	    printm("Loading from %s\n", gExtensionsSpec );
	    FileLoadDrivers(gExtensionsSpec, 0, 0 );
    }
  } else {
    return 0;
  }
...
static long FileLoadDriversMKext( char *dirSpec )
{
	...
	//printm("Loading mkext: %s\n", dirSpec );

	ret = GetFileInfo(dirSpec, "Extensions.mkext", &flags, &time);
	if ((ret == 0) && ((flags & kFileTypeMask) == kFileTypeFlat)) {
		ret = GetFileInfo( dirSpec, "Extensions", &flags, &time2 );

		load_mkext = ret || ((flags & kFileTypeMask) != kFileTypeDirectory);
		if( !load_mkext && ((gBootMode & kBootModeSafe) || time < time2) ) {
			printm("Old mkext timestamp (or safe-boot)\n");
			return -1;
		} else {
			...
			if( LoadDriverMKext(gDriverSpec) == 0 ) 
				return 0;
			printm("Failed to load mkext!\n");
			return -1;
		}
	}
	printm("%sExtensions.mkext is missing\n", dirSpec);
	return -1;
}
...
static long FileLoadDrivers(char *dirSpec, long plugin, int dry_run )
{
  ...
  while (1) {
    ...
    if( !dry_run )
	    LoadDriverPList(dirSpec, gFileName, bundleType);
    if (!plugin)
	    drvcnt += FileLoadDrivers(gDriverSpec, 1, dry_run );
    drvcnt++;
  }
  return drvcnt;
}
...
static long LoadDriverMKext(char *fileSpec)
{
  ...
  if (package->alder32 != Alder32((unsigned char *)&package->version, package->length - 0x10)) {
	printm("mkext has bad alder32 checksum - ");  
	return -1;
  }
...
static long LoadMatchedModules(void)
{
  ...
  while (module != 0) {
	...
	sprintf(segName, "Driver-%lx", (ulong)driver);
...
static long ParseXML(char *buffer, ModulePtr *module, TagPtr *personalities)
{
  ...
  if ((required == 0) || (required->type != kTagTypeString) || !strcmp(required->string, "Safe Boot")) {
	  ...
	  /* XXX: MOL-hack */
	  int load = 0;
	  TagPtr name = GetProperty( moduleDict, kPropCFBundleIdentifier );
	  if( name && name->type == kTagTypeString ) {
		  load = !strcmp(name->string, "com.apple.iokit.IOAudioFamily");
	  }
	  if( !load ) {
		  FreeTag(moduleDict);
		  return -1;
	  }
	  printm("Force-loading %s\n", name->string );
#else
	  FreeTag(moduleDict);
	  return -1;
#endif
