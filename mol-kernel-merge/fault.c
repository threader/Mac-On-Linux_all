...
static inline unsigned long fix_pte(unsigned long * p, unsigned long set, unsigned long flags)
{
	unsigned long ret, tmp;

	/* FIXME Comment this ASM */
...
unsigned long get_phys_page(kernel_vars_t * kv, unsigned long va, int request_rw)
{
	char *lvptr = (char *)va;
	unsigned long lpte, uptr, *ptr;
	unsigned long flags;
	struct mm_struct *mm;
	...
	uptr = ((unsigned long *) current->thread.pgdir)[va >> 22];	/* top 10 bits */
	ptr = (unsigned long *) (uptr & ~0xfff);
	if (!ptr)
		goto no_page;
...
int dbg_get_linux_page(unsigned long va, dbg_page_info_t * r)
{
	unsigned long val, uptr, *ptr;

	/* top 10 bits */
	uptr = ((unsigned long *) current->thread.pgdir)[va >> 22];
	ptr = (unsigned long *) (uptr & ~0xfff);
	if (!ptr)
		return 1;
	ptr = phys_to_virt((int)ptr);
