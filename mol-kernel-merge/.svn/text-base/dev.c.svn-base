...
static int find_physical_rom(int *base, int *size)
{
...
static int get_info(mol_kmod_info_t * user_retinfo, int size)
{
...
void prevent_mod_unload(void)
{
	__module_get(THIS_MODULE);
}

int get_irqs(kernel_vars_t * kv, irq_bitfield_t * irq_info_p)
{
	...
	if (copy_to_user
	    (irq_info_p, &(kv->mregs.active_irqs),
...
static int debugger_op(kernel_vars_t * kv, dbg_op_params_t * upb)
{
	dbg_op_params_t pb;
	int ret;
...
static int mol_open(struct inode *inode, struct file *file)
{
	int ret = 0;
...
static int mol_release(struct inode *inode, struct file *file)
{
...
static int
mol_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	  unsigned long arg)
{
	mol_ioctl_pb_t pb;
	kernel_vars_t *kv;
...
static int __init dev_register(void)
{
	printk("MOL %s kernel module loaded\n", MOL_RELEASE);
	return misc_register(&mol_device);
}

static void __exit dev_unregister(void)
{
