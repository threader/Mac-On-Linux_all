...
int init_mmu_tracker(kernel_vars_t * kv)
{
	/* track_lvrange does the initialization */
	return 0;
}

void cleanup_mmu_tracker(kernel_vars_t * kv)
{
	DECLARE_TS;
	if (!ts)
		return;
...
int track_lvrange(kernel_vars_t * kv)
{
	...
	if (!(ts = kmalloc(sizeof(tracker_data_t), GFP_KERNEL)))
		return 1;
...
size_t get_track_buffer(kernel_vars_t * kv, char *retbuf)
{
	DECLARE_TS;
	...
	if (copy_to_user(retbuf, ts->table, ts->table_size))
		return 0;
	return ts->table_size;
}

void set_track_buffer(kernel_vars_t * kv, char *buf)
{
	DECLARE_TS;
	...
	if (copy_from_user(ts->table, buf, ts->table_size)) {
