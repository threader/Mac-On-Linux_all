 ...
 * clears the bat registers in the idle loop). Of course, this breaks
...
int do_mtsdr1(kernel_vars_t * kv, unsigned long value)
{
	...
	if (MMU.pthash_inuse_bits)
		kfree(MMU.pthash_inuse_bits);
	if (!(MMU.pthash_inuse_bits = kmalloc(s, GFP_KERNEL)))
		MMU.pthash_inuse_bits_ph = 0;
	else {
		memset(MMU.pthash_inuse_bits, 0, s);
		MMU.pthash_inuse_bits_ph = virt_to_phys(MMU.pthash_inuse_bits);
	}
...
int do_mtbat(kernel_vars_t * kv, int sprnum, unsigned long value, int force)
{
	...
	d->vp = p->batu.vp;
	d->wimg =
	    (p->batl.w << 3) | (p->batl.i << 2) | (p->batl.m << 1) | p->batl.g;
...
int mapin_emuaccel_page(kernel_vars_t * kv, int mphys)
{
	...
	if (!(kv->emuaccel_page = get_zeroed_page(GFP_KERNEL)))
		return 0;
