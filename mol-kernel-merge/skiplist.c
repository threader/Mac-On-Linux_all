...
static inline int _cntlz(int val)
{
	int ret;
...
static unsigned long mol_random(void)
{
	unsigned int t;
	asm("mftb %0": "=r"(t):);
...
static void mol_random_entropy(void)
{
...
static inline void set_level_next(skiplist_level_t * level, skiplist_el_t * el)
{
	level->next = el;
}
		  ...
		  skiplist_el_callback callback, void *usr1, void *usr2)
{
	...
	for (count = 0;; size -= s, buf += s, count++) {
		for (n = 0;
		     n < SKIPLIST_MAX_HEIGHT - 1 && (mol_random() & 0x40);
		     n++) ;
		s = sl->datasize + sizeof(skiplist_t) +
		    n * sizeof(skiplist_level_t);
		if (s > size)
			break;
	...
	for (n = 0, p = head; p; p = p->level[0].next, n++) {
		if (callback)
			(*callback) ((char *)p - sl->datasize, n, count, usr1,
				     usr2);
		if (!p->level[0].next) {
...
char *skiplist_insert(skiplist_t * sl, int key)
{
	skiplist_el_t *pleft =
	    (skiplist_el_t *) ((char *)&sl->root[0] -
	...
	/* pick a good search level (-3 is benchmarked) */
	sl->nel++;
	...
	if (slev < n)
		slev = n;
	for (; slev >= 0; slev--) {
		for (; pleft->level[slev].next->key < key;
		     pleft = pleft->level[slev].next) ;
		if (slev <= n) {
...
char *skiplist_delete(skiplist_t * sl, int key)
{
	skiplist_el_t *p =
	    (skiplist_el_t *) ((char *)&sl->root[0] -
			       offsetof(skiplist_el_t, level));
...
char *skiplist_lookup(skiplist_t * sl, int key)
{
	skiplist_el_t *p =
	    (skiplist_el_t *) ((char *)&sl->root[0] -
			       offsetof(skiplist_el_t, level));
...
void skiplist_init(skiplist_t * sl, int datasize)
{
