...
static void flush_lvptr_(vsid_info_t * vi, unsigned long lvptr)
{
	...
	for (lvr = vi->lvrange_head; lvr && lvptr - lvr->base >= lvr->size;
	     lvr = lvr->next) ;
	if (!lvr)
		return;
...
void flush_ea_range(kernel_vars_t * kv, unsigned long org_ea, int size)
{
	...
	while (skiplist_getnext(&MMU.vsid_sl, &iter, &userdata)) {
		...
		while (ea < end) {
			...
			for (i = PELIST_IND(ea); i < 32 && ea < end;
			     i++, ea += 0x1000) {
				if (t->pelist[i])
					flush_vsid_ea_(vi, r, ea);
			}
...
static void *do_chunk_kmalloc(vsid_info_t * vi, int what)
{
	alloc_ent_t *mp;
	char *ptr;
...
static void do_kfree(vsid_info_t * vi, int what)
{
...
static inline pent_table_t *get_free_lev2(vsid_info_t * vi)
{
...
static pterec_t *get_free_pent(vsid_info_t * vi, pte_lvrange_t * lvrange,
			       char *lvptr)
{
	pterec_t *pr, *pr2;
	int pent = 0;
...
static int lev2_alloc(vsid_info_t * vi)
{
...
static int pent_alloc(vsid_info_t * vi)
{
 ...
 */
int mtable_memory_check(kernel_vars_t * kv)
{
	vsid_info_t *vi = MMU.vsid_info;
...
relink_lv(vsid_info_t * vi, pterec_t * pr, pte_lvrange_t * lvrange, char *lvptr)
{
	...
	if (pr->pent & PENT_LV_HEAD) {
		...
		for (p = pr->lv_next; !(p->pent & PENT_LV_HEAD);
		     p = p->lv_next) ;
		if (p == lv_head) {
	     ...
	     unsigned long * pte, vsid_ent_t * r, int segreg)
{
	...
	if ((pr = *pp)) {
		...
		pr->pent |=
		    PTE_TO_IND(pte) | pent_cmp | ((ea & BIT(14)) ? PENT_EA_BIT14
						  : 0);
 ...
 */
static vsid_ent_t *alloc_vsid_ent(kernel_vars_t * kv, int mac_vsid)
{
	char *buf;
	...
	if (skiplist_needalloc(&MMU.vsid_sl)) {
		...
		(void)skiplist_prealloc(&MMU.vsid_sl, buf, CHUNK_SIZE,
					_vsid_el_callback, kv, NULL);
	}
	return (vsid_ent_t *) skiplist_insert(&MMU.vsid_sl, mac_vsid);
}

/* flushes all vsids (including the fake no-MMU vsids) */
void clear_all_vsids(kernel_vars_t * kv)
{
	vsid_info_t *vi = MMU.vsid_info;
	skiplist_iter_t iter;
	char *userdata;
 ...
 */
void clear_pte_hash_table(kernel_vars_t * kv)
{
...
void mtable_reclaim(kernel_vars_t * kv)
{
	vsid_info_t *vi = MMU.vsid_info;
	skiplist_iter_t iter;
	pent_table_t *t;
	...
	while (skiplist_getnext(&MMU.vsid_sl, &iter, &userdata)) {
		...
		if (i == n1) {
			int vsid =
			    skiplist_iter_getkey(&MMU.vsid_sl, (char *)r);
...
pte_lvrange_t *register_lvrange(kernel_vars_t * kv, char *lvbase, int size)
{
	vsid_info_t *vi = MMU.vsid_info;
	pte_lvrange_t *lvr;
	int i, nel = (size >> 12);
	...
	if (!(lvr = kmalloc(sizeof(pte_lvrange_t), GFP_KERNEL)))
		return NULL;
...
void free_lvrange(kernel_vars_t * kv, pte_lvrange_t * lvrange)
{
	vsid_info_t *vi = MMU.vsid_info;
	pte_lvrange_t **lvr;
...
void mtable_tune_alloc_limit(kernel_vars_t * kv, int ramsize_mb)
{
	vsid_info_t *vi = MMU.vsid_info;
...
int init_mtable(kernel_vars_t * kv)
{
	vsid_info_t *vi = kmalloc(sizeof(vsid_info_t), GFP_KERNEL);
...
void cleanup_mtable(kernel_vars_t * kv)
{
	vsid_info_t *vi = MMU.vsid_info;
