...
int common_init(void)
{
	...
	if (!(g_sesstab = kmalloc(sizeof(*g_sesstab), GFP_KERNEL))) {
		cleanup_hash();
		return 1;
	}
...
int initialize_session(uint index)
{
	int ret;
...
void destroy_session(uint index)
{
	kernel_vars_t *kv;
	...
	if (g_sesstab->magic == 1) {
		printk
		    ("Security alert! Somebody other than MOL has tried to invoke\n"
...
uint get_session_magic(uint random_magic)
{
	if (random_magic < 2)
		random_magic = 2;
